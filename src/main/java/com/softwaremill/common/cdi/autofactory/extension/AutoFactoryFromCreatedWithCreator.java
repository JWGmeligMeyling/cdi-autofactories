...
public class AutoFactoryFromCreatedWithCreator<T> {
    ...
    private InjectionTarget<T> createInjectionTargetWithoutValidation() {
        BeanManagerImpl bmi = ((BeanManagerProxy) beanManager).delegate();
        ClassTransformer classTransformer = bmi.getServices().get(ClassTransformer.class);
        EnhancedAnnotatedType<T> enhancedAnnotatedType = classTransformer
            .getEnhancedAnnotatedType((SlimAnnotatedType<T>) createdType);
        return new InjectionTargetFactoryImpl<T>(createdType, bmi){}
            .createInjectionTarget(enhancedAnnotatedType, null, false);
    }
    ...
    private AnnotatedConstructor<T> getSoleAcceptableConstructorOfCreatedType() {
        ...
        final Predicate<AnnotatedConstructor<T>> diConstructorPredicate = new Predicate<AnnotatedConstructor<T>>() {
            @Override
            public boolean apply(AnnotatedConstructor<T> input) {
